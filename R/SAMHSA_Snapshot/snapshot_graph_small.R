...
mydata <- myfile[, c(1,2,3,7,8,11,12,13,14)]  # get the columns we want, small data only
#View(mydata)
...
mygraph <- ggplot(my_clean_data) + geom_line(aes(x=Date, y=value, colour=variable), size=1.5) +
  scale_colour_manual(values=c("#2ca02c", "#d62728", "#9467bd", "#8c564b",
                               "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"))

end_date <- max(my_clean_data$Date)  # get the latest end date to limit what gets plotted

# Formatting for plot
mygraph <- mygraph +  xlab("Date") + ylab("Calls/Chats Answered") +
  ggtitle("Summary by Month (All Other) - CY 2014") +
  expand_limits(y=0) +  # Force chart to go down to 0
  scale_y_continuous(labels = comma, limits=c(0, 7000), expand=c(0,0),  # expand removes negative area
